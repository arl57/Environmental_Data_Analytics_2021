...
2. Q&A on M09 Recordings/Concepts
...
wind <- read.csv('https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv', colClasses = c('HUC_12' = 'factor')) %>%
  mutate('HUC_8' = substr(HUC_12, 1, 8)) %>%
  group_by(HUC_8) %>%
  summarize(power = mean(kWhkm2day))

#Join to HUC_8 features
all.data <- left_join(huc8_sf, wind, by = "HUC_8")
