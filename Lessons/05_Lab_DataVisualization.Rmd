...
ggplot(subset(PeterPaul.chem.nutrients, depth == 0), aes(x = daynum, y = temperature_C, color = year4)) +
  geom_line() +
  facet_wrap(~lakename, nrow=2) +
  labs(x = "Day of Year", y = "Temperture (Celsius)")
...
ggplot(PeterPaul.chem.nutrients, aes(x = sampledate, y = temperature_C, color = depth)) +
  geom_point(size = 0.5) +
  scale_color_viridis_c(begin = 1, end = 0, option = "magma") +
  scale_x_date(date_breaks = "5 years", date_labels = "%Y") +
  labs(x = "Date", y = "Temperture (Celsius)")
...
ggplot(EPAair, aes(x = PM2.5, y = Ozone, color = SITE_LATITUDE)) +
  geom_point(alpha = 0.5) +
  geom_smooth(method = lm, color = "black") +
  labs(x = "PM2.5 AQI", y = "Ozone AQI")
...
EPAair$Year <- as.character(EPAair$Year)

ggplot(EPAair, aes(x = Year, y = PM2.5)) +
  geom_boxplot() 
